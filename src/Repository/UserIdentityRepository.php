...
namespace BlackCat\Database\Packages\UserIdentities\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\UserIdentities\Definitions;
use BlackCat\Database\Packages\UserIdentities\Criteria;
use BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto as Dto;
use BlackCat\Database\Packages\UserIdentities\Mapper\UserIdentityDtoMapper as RowMapper;
...
final class UserIdentityRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\UserIdentities\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'provider', 'provider_user_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\UserIdentities\Definitions::table());
            return $bulk->upsertMany($rows, [ 'provider', 'provider_user_id' ], [ 'user_id' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto|null */
    public function getByProviderAndProviderUserId(string $provider, string $providerUserId, bool $asDto = false): array|\BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto|null {
        $row = $this->getByUnique([ 'provider' => $provider, 'provider_user_id' => $providerUserId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\UserIdentities\Mapper\UserIdentityDtoMapper::fromRow($row);
    }
    public function existsByProviderAndProviderUserId(string $provider, string $providerUserId): bool {
        $where = 't.' . Ident::q($this->db, 'provider') . ' = :uniq_provider' . ' AND ' . 't.' . Ident::q($this->db, 'provider_user_id') . ' = :uniq_provider_user_id';
        return $this->exists($where, [ 'uniq_provider' => $provider, 'uniq_provider_user_id' => $providerUserId ]);
    }
    /** @return int|string|null */
    public function getIdByProviderAndProviderUserId(string $provider, string $providerUserId) {
        $row = $this->getByProviderAndProviderUserId($provider, $providerUserId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto|null {
