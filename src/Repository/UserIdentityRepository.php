...
use BlackCat\Database\Packages\UserIdentities\Repository\UserIdentityRepositoryInterface;
...
class UserIdentityRepository implements UserIdentityRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'provider', 'provider_user_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\UserIdentities\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'user_id' ]);
          return count($rows);
        }
    ...
    public function getByProviderAndProviderUserId(string $provider, string $providerUserId, bool $asDto = false): array|\BlackCat\Database\Packages\UserIdentities\Dto\UserIdentityDto|null {
        return $this->getByUnique([ 'provider' => $provider, 'provider_user_id' => $providerUserId ], $asDto);
    }
    ...
    public function getIdByProviderAndProviderUserId(string $provider, string $providerUserId) {
        $row = $this->getByProviderAndProviderUserId($provider, $providerUserId, false);
